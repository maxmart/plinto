...
/** A synced copy kept by the translation sync, never content in its own right. */
const isSyncedCopy = (path: string) => /(^|\/)\.obelum\//.test(path);
  ...
  async function pull(
    ...
    try {
      ...
      if (result.conflicts.length > 0) {
        ...
        let committed = false;
        try {
          // A conflicting synced copy under .obelum/ is not content to merge:
          // both sides synced the same language against overlapping edits.
          // Picking either side would claim that language is in sync with a
          // file it never saw (a false in-sync, silent). Pointing the copy at
          // the merge base says the truth: the language has seen neither
          // side's edit, and its next sync incorporates both. With no base
          // at all, an empty copy reads as never synced, which is full
          // evidence — one extra translation round, never a missed one.
          const copies = result.conflicts.filter(c => isSyncedCopy(c.path));
          const documents = result.conflicts.filter(c => !isSyncedCopy(c.path));
          ...
          await gitStore.completeMerge(resolved);
          committed = true;
        } finally {
