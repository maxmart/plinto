...
describe('a merge with conflicts', () => {
  ...
  it('returns the conflicted files and commits nothing', async () => {
    ...
    // The base rides along: a conflicting synced copy under .obelum/ is
    // resolved to it rather than to either side (see ops/repo).
    expect(result.conflicts[0].base).toBe('base\n');
