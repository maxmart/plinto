  ...
  async function threeWayMerge(
    ...
    try {
      ...
      for (const path of allPaths) {
        ...
        // Real conflict — return to caller for resolution, with the base so
        // a caller can fall back to what both sides started from.
        const base = baseOid ? new TextDecoder().decode(await readBlob(baseOid)) : undefined;
        conflicts.push({ path, ours: ourContent, theirs: theirContent, ...(base !== undefined ? { base } : {}) });
      }
