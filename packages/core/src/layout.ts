...
import { collapseDots } from './storage/path-normalize';
...
/**
 * Where a language keeps its synced copy of a file: at the repository root,
 * under `.obelum/{viewer}/`, at the file's own repository path — the
 * viewer's checkout of the whole repository, one directory like `.git`.
 *
 *   .obelum/sv/src/pages/no/pricing.mdx                  sv's copy of no's page
 *   .obelum/sv/sites/cupmanager/src/pages/no/pricing.mdx  the same, in a monorepo
 *   .obelum/sv/content-shared/staff/no/anna.mdx           a collection two sites share
 *
 * Paths in and out are site-relative, as every path the stores take is; a
 * site below the repository root reaches the directory with `../`.
 */
export function syncedFilePath(c: ResolvedConfig, realPath: string, viewer: string): string {
  const subdir = (c.content.siteSubdir ?? '').replace(/^\/+|\/+$/g, '');
  const repoPath = collapseDots(subdir ? `${subdir}/${realPath}` : realPath);
  const up = subdir ? '../'.repeat(subdir.split('/').length) : '';
  return `${up}.obelum/${viewer}/${repoPath}`;
}
