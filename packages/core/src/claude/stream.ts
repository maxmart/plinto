/**
 * The Claude agent loop for merge-conflict resolution.
 *
 * Ask the model, surface its text as it arrives, run the tools it calls,
 * replay the conversation with the results, repeat until it stops. The
 * conflict agent supplies the prompt, the tools, and what to do when a tool
 * is called. Translation sync used to share this loop; it now lives in
 * @obelum/translator-claude, which carries a copy of the same machine so that
 * package can be used on its own.
 *
