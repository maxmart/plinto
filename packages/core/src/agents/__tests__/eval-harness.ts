 ...
 * documents are seeded, marked as synced to each other and committed, the
 * scenario's "edit" is applied through the real `editContent` (so the copies
 * it leaves behind are real history), and
 * `runTranslation` then runs over the real content ops, the real unified
 ...
 *                     staleness, the sync's fan-out — is asserted on
...
export interface ScenarioSpec {
  ...
  target: string;
  /** Locales seeded from the shipped corpus, each marked as synced to the
   *  others — parity, so every structural difference between them is
   *  *accepted editorial drift*, which is precisely what the agent must not
   *  undo. */
  seed: string[];
  /** The one edit whose translation is being evaluated, applied through the
   *  real editContent so the copies it leaves behind are real history. */
...
export async function buildScenario(spec: ScenarioSpec): Promise<Scenario> {
  ...
  // Seed: shipped corpus bytes, committed as real history.
  const author = { name: 'Eval', email: 'eval@plinto.local' };
  const seeded: Record<string, string> = {};
  for (const lang of spec.seed) {
    const bytes = shippedDoc(filePath(lang));
  ...
  // Parity: every seeded locale is synced to every other as shipped, so each
  // structural difference between them is *accepted editorial drift*, which
  // is precisely what the agent must not undo. Through the real ops, so the
  // copies under .obelum/ are real history.
  for (const lang of spec.seed) await ops.markAsSynced(contentPath, lang);

  // The edit under evaluation, through the front door, committed.
  if (spec.edit) {
