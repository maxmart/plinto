 ...
 *     diff, and that saving leaves the target synced to every source.
...
// Locale-only page: sv/skargarden exists only in Swedish. Translating it into
// en is creation, not edit — full-sync mode, write_file.
...
/** The sync's bookkeeping: the target is synced to every language as it
 *  stands, and no sibling was made stale by the translation. */
async function expectSynced(scenario: Scenario) {
  const stale = await scenario.deps.document(scenario.contentPath).stale();
  expect(stale[scenario.spec.target]).toEqual({ missing: false, stale: [] });
  for (const lang of scenario.spec.seed) expect(stale[lang].stale).not.toContain(scenario.spec.target);
}
...
describe.each(CASES)('pipeline: $spec.name', ({ spec }) => {
  it('shows the agent a minimal diff and leaves the target synced', async () => {
    ...
    try {
      ...
      // Diff mode was reached — the target's copies are the base — and only
      // the edited language contributes.
      ...
      // The probe made no edits, so the save is the pure bookkeeping path:
      // the target untouched byte for byte, its copies snapped to the sources.
      expect(scenario.read(spec.target)).toBe(scenario.targetBefore);
      await expectSynced(scenario);
    } finally {
      scenario.dispose();
    }
  });
...
describe.each(CASES)('agent: $spec.name', ({ spec, mustSurvive }) => {
  ...
  run(`makes a minimal, surgical edit (${driver.mode === 'skip' ? `no fixture at ${fixturePath(spec.name)} and no ANTHROPIC_API_KEY` : driver.mode})`, async () => {
    ...
    try {
      ...
      expect(after).not.toBe(scenario.targetBefore);
...
describe(`agent: ${CREATION.name}`, () => {
  ...
  run(`creates the missing locale without touching the source (${driver.mode === 'skip' ? `no fixture at ${fixturePath(CREATION.name)} and no ANTHROPIC_API_KEY` : driver.mode})`, async () => {
    ...
    try {
      ...
      // Creation bookkeeping: the newborn is synced to sv, and sv is synced
      // to the newborn rather than told it is news.
      await expectSynced(scenario);
    } finally {
      scenario.dispose();
    }
