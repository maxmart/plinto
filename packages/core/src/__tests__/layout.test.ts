...
describe('syncedFilePath', () => {
  const c = testConfig;

  it('is the viewer plus the real path, in one directory at the repository root', () => {
    expect(layout.syncedFilePath({ ...c, content: { ...c.content, siteSubdir: '' } }, 'src/pages/no/pricing.mdx', 'sv'))
      .toBe('.obelum/sv/src/pages/no/pricing.mdx');
  });

  it('reaches the root from a site below it, and keeps a shared collection at its repository path', () => {
    const mono = { ...c, content: { ...c.content, siteSubdir: 'sites/cupmanager' } };
    expect(layout.syncedFilePath(mono, 'src/pages/no/pricing.mdx', 'sv'))
      .toBe('../../.obelum/sv/sites/cupmanager/src/pages/no/pricing.mdx');
    expect(layout.syncedFilePath(mono, '../../content-shared/staff/no/anna.mdx', 'sv'))
      .toBe('../../.obelum/sv/content-shared/staff/no/anna.mdx');
  });
});
