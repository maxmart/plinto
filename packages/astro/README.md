...
- **Browser-side git** (isomorphic-git + lightning-fs): the *deployed static
  ...
  backend.
- **Translation sync**: every language keeps a copy of every sibling's file
  as of the last time it looked, committed under `.obelum/` at the
  repository root. When a sibling's file differs from the copy, the language is
  stale, and a Claude agent translates *what changed* — not the whole page.
  The documents themselves carry no bookkeeping. Conflicting concurrent
  edits are merged by an agent that asks the editor plain-language questions
  when it cannot decide; a conflicting copy under `.obelum/` is never
  merged, only reset to what both sides started from, so the language reads
  as behind on both edits rather than falsely in sync.
...
Everything in the file is yours; the translation sync keeps no bookkeeping
in it. The editor derives the import block from your block registry, so the
file stays valid however it is written.

### The synced copies

At the repository root sits one `.obelum/` directory, like `.git`, holding
for every language a copy of every language's file as of the last time it was
synced, at the file's own repository path:

```
.obelum/sv/src/pages/en/streaming.mdx     the English page as Swedish last saw it
.obelum/sv/src/pages/sv/streaming.mdx     the Swedish page at its last sync
.obelum/sv/content/news/en/summer-cup.mdx
.obelum/sv/sites/cupmanager/src/pages/en/streaming.mdx   in a monorepo
```

`.obelum/sv/` is Swedish's checkout of the whole repository, as far as it
knows. A language is stale when a sibling's real file differs from its copy,
and what the translator is shown is that difference. Saving with **Edit**
leaves the copies alone, so siblings go stale; **Fix** merges the change into
their copies, so a correction or deliberately local content is never news; a
sync snaps the language's copies to the real files. A language's copy of
*itself* is what a translator is told to preserve when that language is the
target, so it moves only when the language is synced, or when a sync round
(Obelum's `syncAll`) has carried an edit to every sibling and the edit is
behind everyone. Nothing sits under `src/` or the content directories, so the
site never sees the copies. Delete `.obelum/` and the site is byte-identical;
every language then reads as never synced and one full round rebuilds it.

Two lines worth adding to the repository's `.gitattributes`, so the copies
collapse in GitHub diffs and `.mdx` files keep the line endings the copies
compare on:
...
A site coming from the older frontmatter bookkeeping (`rev` / `synced`)
migrates in one commit with `scripts/migrate-to-obelum-copies.mjs`: it strips
those lines and writes the first copies, every language synced to every other
as the files stand.
...
@obelum/core           the synced copies: staleness, the translator's
                      brief, the fix fan-out. No I/O.
```
