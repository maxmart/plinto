...
import type { Round } from '@plinto/core/ops/content';
 ...
 *
 * Each document's syncs are one round (Obelum's `Session.syncAll`): the
 * round is opened before its first sync and closed after its last, which
 * lets an edited language's own copy of itself catch up once every sibling
 * has the edit. A cancelled queue leaves its rounds open, and those copies
 * keep lagging until a round completes.
 */
export async function runTranslationQueue(
  plinto: Pick<QueueDeps, 'agents' | 'ops'>,
  items: QueueItem[],
  apiKey: string,
  opts: {
    isCancelled: () => boolean;
  ...
  const roundFor = (contentPath: string) => {
    let round = rounds.get(contentPath);
    if (!round) { round = plinto.ops.document(contentPath).syncAll(); rounds.set(contentPath, round); }
    return round;
  };
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const round = roundFor(items[i].contentPath);
    const result = await runTranslation(
      { contentPath: items[i].contentPath, targetLang: items[i].targetLang, apiKey, sync: round.sync },
      {
