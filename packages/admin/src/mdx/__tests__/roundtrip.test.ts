...
describe('one frontmatter writer', () => {
  it('is a fixpoint for a page with frontmatter', () => {
    // The editor is the only thing that writes a document's frontmatter now;
    // the translation sync keeps its bookkeeping in copies under .obelum/
    // and never touches the file. Opening and saving a page must change
    // nothing at all — down to the last byte, the newline it ends on.
    const page = doc(
      [{ type: 'CmHero', props: { id: 'CmHero-1', title: 'Hej' } }],
      { title: 'Hej', layout: '@/layouts/LangLayout.astro' },
    );
    const parsed = read(page);
    expect(parsed.frontmatter.title).toBe('Hej');
    expect(puckToMdx(parsed.data, parsed.frontmatter, gen)).toBe(page);
  });
});
